...
To size the **burden** resistance, the target voltage level at the sensor output must be considered so that the input range of the analog-to-digital converter (ADC) of the [ESP32](central_controller), typically 0 to 3.3 V, is not exceeded. Consequently, the voltage generated by the secondary current through the **burden** must be, at most, equal to or less than the ADC limit. In AC applications, it is common to add a DC bias at the midpoint of the ADC scale to center the signal within the 0–3.3 V range and avoid clipping.

![[Sensor_and_conditioning.drawio.svg]]
...
A 100 nF capacitor is added as an anti-aliasing filter and another 10 µF capacitor to keep the bias voltage stable. Like the [voltage sensor](voltage_sensor), the sensor and its conditioning circuit can only measure a single-phase signal. To compensate for this and obtain two-phase or three-phase measurements, the use of three sensors with their respective conditioning circuits is proposed; in this way, each one is responsible for one voltage line and complete measurements are obtained.