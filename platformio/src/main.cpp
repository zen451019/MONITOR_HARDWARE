...
// ===================== DISPLAY DATA =====================
void updateDisplay(String systemStatus, float batteryVolt, float volt1 = 0,
                   float volt2 = 0, float volt3 = 0, float curr1 = 0,
                   float curr2 = 0, float curr3 = 0) {
      ...
      if (sistema_habilitado &&
        ...
        for (int i = 0; i < NUM_PINES; ++i) {
          ...
          if (pin_configs[i].pin ==
              pines_voltaje[0]) { // Tomamos solo el primer pin de voltaje como
                                  // referencia
...
void TaskDisplay(void *pvParameters) {
  ...
  display.setCursor(26,
                    0); // mover un poco a la derecha para no chocar con el logo
  display.print("NEMO");
  ...
  static float ultimo_valor_bateria = 0; // Nuevo: valor persistente

  while (true) {
    if (xQueueReceive(queueDisplayInfo, &ultimo_evento, portMAX_DELAY) ==
        pdTRUE) {
      // Si hay nueva muestra, actualiza el valor persistente
      ultimo_valor_bateria = ultimo_evento.bateria_incluida
                                 ? ultimo_evento.valor_bateria
                                 : ultimo_valor_bateria;

      String systemStatus =
          ultimo_evento.sistema_activo ? "ACTIVO" : "INACTIVO";
      float batteryVolt = ultimo_valor_bateria;
      float volt1 = ultimo_evento.sistema_activo
                        ? ultimo_evento.primer_valor_voltaje1
                        : 0;
      float volt2 = 0;
      float volt3 = 0;
      float curr1 = ultimo_evento.sistema_activo
                        ? ultimo_evento.primer_valor_corriente
                        : 0;
      float curr2 = 0;
      float curr3 = 0;
      updateDisplay(systemStatus, batteryVolt, volt1, volt2, volt3, curr1,
                    curr2, curr3);
    }
  }
}
