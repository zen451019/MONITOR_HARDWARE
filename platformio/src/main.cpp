 ...
 * @version     1.0
 *
 * @brief       Firmware for NEMO (Node for Electromechanical Monitoring &
 *Operation).
 * @details     This firmware runs on an ESP32 and is designed for the
 *acquisition, processing, and transmission of analog signals (current/voltage).
 *It features an OLED display for real-time visualization and uses LoRaWAN for
 *data uplink.
 *
 * --------------------------------------------------------------------------------------------------------------------
 *
 ...
 * System Architecture:
 * --------------------
 * The system uses FreeRTOS to manage concurrent tasks for data acquisition,
 *processing, and communication.
 * - An ISR (`onADCTimer`) samples ADC pins at a high frequency (`FS_HZ`) and
 *stores data in a circular FIFO buffer.
 * - `TaskProcesamiento`: Periodically calculates RMS values from the FIFO,
 *applies an adaptive EMA filter, and sends the results to a processing queue.
 * - `TaskRegistroResultados`: Collects processed results into blocks. Once a
 *block is full or the system is disabled, it encodes the data into a bit-packed
 *LoRaWAN payload.
 * - `tareaLoRa`: Manages the LoRaWAN communication, sending payloads from a
 *queue and handling TX/RX events.
 * - `TaskBatteryLevel`: Periodically measures the battery voltage.
 * - `TaskDisplay`: Manages the OLED screen, showing a boot sequence and a log
 *of recent LoRaWAN transmissions.
 * - `TaskMonitorPin`: Controls the system's global enable/disable state based
 *on a physical pin.
 *
 * Main Features:
 * --------------
 ...
 * - Data blocking and bit-packed payload encoding for efficient LoRaWAN
 *transmission.
 ...
 * --------|------------|-----------------|----------------------------------------------------
 * 1.0     | 2025-09-03 | Manuel F. Ospina| Changed LoRa message transmission to
 *a unified |            |                 | message model, replacing previous
 *fragmented system.
 * --------|------------|-----------------|----------------------------------------------------
 * 1.1     | 2025-09-05 | Manuel F. Ospina| Redesigned payload format into a
 *unified, scalable |            |                 | and modular message model
...
const unsigned char epd_bitmap_[] PROGMEM = {
    // 'Asset 3, 20x20px
    0x00, 0x00, 0x00, 0x07, 0xfe, 0x00, 0x1f, 0xff, 0x80, 0x3f, 0xff, 0xc0,
    0x7e, 0x07, 0xe0, 0x78, 0x01, 0xf0, 0x71, 0xf8, 0xe0, 0x03, 0xfc, 0x00,
    0x07, 0xff, 0x00, 0x0f, 0xff, 0x00, 0x0f, 0x0f, 0x00, 0x06, 0x02, 0x00,
...
// Nueva estructura de datos para la comunicación con la pantalla.
...
// Cola para enviar la información a la tarea de display.
...
#define SENSOR_DATA_MAX_LEN \
    10 // Máximo 10 bytes de datos por sensor (muy flexible)
...
float applyEMA_adsoluto(float input, float &previous_output)
{
    ...
    const float delta_max =
        15.0f; // Cambio máximo esperado (ajusta según tu señal)
...
float applyEMA(float input, float &previous_output)
{
    ...
    float delta_rel =
        fabs(input - previous_output) / (fabs(previous_output) + epsilon);
    float alpha =
        alpha_min + (alpha_max - alpha_min) * (delta_rel / delta_rel_max);
    if (alpha > alpha_max)
        alpha = alpha_max;
    if (alpha < alpha_min)
        alpha = alpha_min;
...
// ===================== DISPLAY DATA (VERSIÓN OPTIMIZADA) =====================
void updateDisplay(String systemStatus, float batteryVolt, unsigned long timestamp,
                   int volt1 = 0, float curr1 = 0)
{
    ...
    // --- Fila superior: Estado y Timestamp ---
    display.setCursor(0, 7);
    display.print(systemStatus);

    // Mover timestamp a la derecha
    char ts_buf[20];
    unsigned long hours = (timestamp / 3600) % 24;
    unsigned long minutes = (timestamp / 60) % 60;
    unsigned long seconds = timestamp % 60;

    sprintf(ts_buf, "%02lu:%02lu:%02lu", hours, minutes, seconds);
    display.setCursor(128 - (strlen(ts_buf) * 4), 7);
    display.print(ts_buf);
...
void codificarUnificado(
    const BufferResultados &buffer, uint8_t id_mensaje,
    ...
    std::vector<Fragmento> &fragmentos)
{
    ...
    // Timestamp
    unsigned long ts_s = (sistema_habilitado)
                             ? (buffer.bloque[0].timestamp / 1000)
                             : (millis() / 1000);
    ...
    if (sistema_habilitado)
        activate_byte |= (1 << 1); // voltaje
    if (sistema_habilitado)
        activate_byte |= (1 << 2); // corriente
    ...
    for (int i = 0; i < MAX_SENSORES_EXTERNOS; ++i)
    {
        ...
        {
            uint8_t len_byte = 0;
            if (datos_externos[i].packed)
                len_byte |= 0x80;
            if (datos_externos[i].extended)
                len_byte |= 0x40;
    ...
    {
        // ASUNCIÓN: pin_configs[3] es la única corriente. Rellenamos 2 canales con
        // ceros.
        int idx_corriente = 3;
        // Canal 1 (real)
        for (int k = 0; k < data_len_rms; ++k)
        {
            uint16_t valor = 0;
            if (pin_configs[idx_corriente].enabled &&
                !isnan(buffer.bloque[k].valores[idx_corriente]))
            {
                valor =
                    (uint16_t)round(buffer.bloque[k].valores[idx_corriente] * 10.0f);
            }
    ...
    for (int i = 0; i < MAX_SENSORES_EXTERNOS; ++i)
    {
        if (datos_externos[i].is_new)
        {
            payload.insert(payload.end(), datos_externos[i].data,
                           datos_externos[i].data + datos_externos[i].len);
        }
    }
...
void TaskRegistroResultados(void *pvParameters)
{
    ...
    {
        ResultadoRMS resultado;
        if (xQueueReceive(queueResultados, &resultado, pdMS_TO_TICKS(500)) ==
            pdTRUE)
        {
            ...
            if (sistema_habilitado &&
                bufferResultados.index >= RESULTADOS_POR_BLOQUE)
            {
                ...
                for (int i = 0; i < MAX_SENSORES_EXTERNOS; ++i)
                {
                    datos_externos_para_envio[i].is_new =
                        false; // Por defecto, no hay datos nuevos
                    if (xSemaphoreTake(mutex_external_sensors[i], pdMS_TO_TICKS(10)) ==
                        pdTRUE)
                    {
                        if (external_sensors[i].is_new)
                        {
                            memcpy(&datos_externos_para_envio[i], &external_sensors[i],
                                   sizeof(ExternalSensorData));
                ...
                codificarUnificado(
                    bufferResultados, id_mensaje, nueva_bateria,
                ...
                DisplayInfo info = {};
                info.timestamp_s =
                    bufferResultados.bloque[RESULTADOS_POR_BLOQUE - 1].timestamp / 1000;
                ...
                for (int i = 0; i < NUM_PINES; ++i)
                {
                    ...
                    if (pin_configs[i].pin ==
                        pines_voltaje[0])
                    { // Tomamos solo el primer pin de voltaje como
                      // referencia
        ...
        if (!sistema_habilitado)
        {
            ...
            if (index_bateria > 0)
            {
                ...
                std::vector<Fragmento> frags;
                codificarUnificado(
                    bufferResultados, id_mensaje, true, nivel_bateria,
                    RESULTADOS_POR_BLOQUE,
                ...
                // <<< CAMBIO: Preparar y enviar datos a la tarea de display (solo
                // batería)
                DisplayInfo info = {};
...
void TaskBatteryLevel(void *pvParameters)
{
    ...
    while (true)
    {
        ...
        float voltage = (raw / 4095.0f) * 3.3f *
                        voltage_divider_ratio; // ← ajusta según divisor real
...
void TaskDisplay(void *pvParameters)
{
    ...
    display.print("V1.1"); // Actualicé la versión como sugerencia
    ...
    float ultimo_valor_bateria = 0.0f;

    while (true)
    {
        // Espera por nueva información para mostrar
        if (xQueueReceive(queueDisplayInfo, &current_info, portMAX_DELAY) == pdTRUE)
        {

            // Si el nuevo dato incluye batería, actualizamos nuestro valor persistente
            if (current_info.bateria_incluida)
            {
                ultimo_valor_bateria = current_info.valor_bateria;
            }

            String systemStatus = current_info.sistema_activo ? "ACTIVO" : "INACTIVO";

            // Si el sistema está inactivo, los valores de los sensores son 0
            int volt1 = current_info.sistema_activo ? current_info.primer_valor_voltaje1 : 0;
            float curr1 = current_info.sistema_activo ? current_info.primer_valor_corriente : 0;
...
void onEvent(ev_t ev)
{
    if (ev == EV_TXCOMPLETE)
    {
        lora_tx_done = true;
        xSemaphoreGive(
            semaforoEnvio); // Libera el semáforo para el siguiente fragmento
...
void tareaLoRa(void *pvParameters)
{
    ...
    while (true)
    {
        ...
        if (xQueueReceive(queueFragmentos, &frag, portMAX_DELAY) == pdTRUE)
        {
            ...
            // 2. Toma el semáforo para asegurar que no se envíe nada más hasta que
            // termine
...
void setup()
{
    ...
    // --- INICIALIZACIÓN DE I2C CON ALTA VELOCIDAD ---
    Wire.begin();           // Inicia el bus I2C con los pines por defecto
    Wire.setClock(400000L); // Establece la velocidad a 400kHz
    ...
    queueResultados =
        xQueueCreate(RESULTADOS_POR_BLOQUE * 2, sizeof(ResultadoRMS));
    queueFragmentos = xQueueCreate(10, sizeof(Fragmento));
    queueDisplayInfo =
        xQueueCreate(5, sizeof(DisplayInfo)); // <<< CAMBIO: Crear la nueva cola
    ...
    adcTimer =
        timerBegin(0, 80, true); // Prescaler = 80 → tick de 1 µs (80 MHz / 80)
    ...
    xTaskCreatePinnedToCore(TaskProcesamiento, "Procesamiento", 4096, NULL, 1,
                            NULL, 0);
    xTaskCreatePinnedToCore(TaskRegistroResultados, "RegistroResultados", 4096,
                            NULL, 1, NULL, 0);

    // --- Tarea de monitoreo de batería ---
    mutex_bateria = xSemaphoreCreateMutex();
    xTaskCreatePinnedToCore(TaskBatteryLevel, "BatteryLevel", 2048, NULL, 0, NULL,
                            0);
